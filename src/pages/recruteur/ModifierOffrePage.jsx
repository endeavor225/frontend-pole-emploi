...
import { Loader2, ArrowLeft, CalendarIcon, Save } from "lucide-react";
...
export default function ModifierOffrePage() {
  ...
  const { offre, isLoading: offreLoading, mutate } = useOffre(id);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      experienceMin: offre?.experienceMin ?? offre?.experience_min ?? 0,
      salaireMin: offre?.salaireMin ?? offre?.salaire_min ?? "",
      salaireMax: offre?.salaireMax ?? offre?.salaire_max ?? "",
      typeOffre: offre?.typeOffre || "",
      localisation: offre?.localisation || "",
      dateLimite: offre?.dateLimite ? parseISO(offre.dateLimite) : null,
      domaineIds: offre?.domaines?.map((d) => d.id) || [],
      niveauxEtudeIds:
        offre?.niveauxEtude?.map((n) => n.id) ||
        offre?.niveauEtudes?.map((n) => n.id) ||
        [],
    },
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        if (payload.salaireMin === "" || isNaN(payload.salaireMin))
          payload.salaireMin = null;

        // Formatage de la date ISO vers YYYY-MM-DD
        ...
        mutate(); // Recharger les données SWR
  ...
  return (
    <div className="max-w-4xl mx-auto pb-10">
      ...
      <div className="flex items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground">
            Mettez à jour les détails de votre offre de recrutement
          </p>
        </div>
      </div>
      ...
      <form onSubmit={formik.handleSubmit} className="space-y-6">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select
                  key={formik.values.typeOffre || "empty"}
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
                      ...
                      disabled={(date) =>
                        ...
                        date <
                          parseISO(
                            offre?.dateLimite || new Date().toISOString(),
                          )
                      }
                      initialFocus
                      locale={fr}
        ...
        {/* Section 3 : Rémunération et Délai */}
        <Card className="border-border/50 shadow-sm">
          <CardHeader>
            <CardTitle className="text-xl">Rémunération</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
        ...
        <div className="flex items-center gap-4 pt-4">
          <Button
            ...
            variant="outline"
            size="lg"
            className="h-12 px-8 flex-1 sm:flex-none border-border hover:bg-destructive hover:text-destructive-foreground"
            onClick={() => navigate(-1)}
            disabled={formik.isSubmitting}
          >
            Annuler
